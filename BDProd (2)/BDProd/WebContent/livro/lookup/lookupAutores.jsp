...
<f:subview id="lookupAutores">
	...
	<div id="dialogAutorExterno"
		...
		<h:panelGroup id="panelAutorExterno">
			...
			<h:inputText id="nomeProjetoSimples" size="93"
				value="#{publicacaoHandler.autorExterno.nome}" />
			<br />
			<br />
			<h:outputLabel value="Nome na Publicação:" />
			<br />
			<h:inputText size="50"
				value="#{publicacaoHandler.autorExterno.nomePublicacao}" />
			<br />
			<br />
		</h:panelGroup>
		<br />
		<div align="center"><h:panelGrid columns="2">
			...
			<a4j:commandLink
				onclick="jQuery('#dialogAutorExterno').dialog('close')"
				action="#{publicacaoHandler.adicionarAutorExternoPublicacao}"
				reRender="autores" oncomplete="#{rich:component('dsAutor')}.last()">
	...
	<div id="dialogTipoAutor" title="Escolha um tipo do Autor"><h:form
		styleClass="niceform">
		<div align="center"><h:selectOneRadio id="tipoAutor"
			value="#{publicacaoHandler.tipoAutor}">
			<f:selectItem itemValue="Externo" itemLabel="Autor externo" />
			<f:selectItem itemValue="LIKA" itemLabel="Autor do LIKA" />
			<a4j:support event="onclick" reRender="panelTipo"></a4j:support>

		</h:selectOneRadio> <h:panelGrid columns="2" id="panelTipo">

			<a4j:commandLink onclick="jQuery('#dialogTipoAutor').dialog('close')">
				<h:graphicImage value="../css/images/cancelar.png" />
			</a4j:commandLink>

			<a4j:commandLink action="#{publicacaoHandler.novoAutorExterno}"
				reRender="panelAutorExterno"
				onclick="jQuery('#dialogTipoAutor').dialog('close') "
				oncomplete="jQuery('#dialogAutorExterno').dialog('open')"
				rendered="#{publicacaoHandler.tipoAutor == 'Externo'}">
				<h:graphicImage value="../css/images/adicionar.png" />
			</a4j:commandLink>
			...
			<a4j:commandLink
				...
				rendered="#{publicacaoHandler.tipoAutor == 'LIKA'}">
				<h:graphicImage value="../css/images/adicionar.png" />
			</a4j:commandLink>
		</h:panelGrid></div>
	</h:form></div>
			...
			<rich:dataTable id="tabelaResultado" value="#{pessoaHandler.pessoas}"
				...
				<rich:column id="opcoes">
					...
					<div style="width: 60px" align="center"><a4j:commandLink
						ajaxSingle="true" oncomplete="#{rich:component('dsAutor')}.last()"
						onclick="jQuery('#dialogAutores').dialog('close') "
						value="Selecionar"
						actionListener="#{publicacaoHandler.adicionarAutorDoLIKAPublicacao}"
						reRender="autores">
