	...
	<body>
	    <div id="container">
			...
			<div id="contentListar" >
				<h2>Laudos (HBV)</h2><!-- campo obrigatorio o h2 no css!!! -->

				<div id='resultadoListar'>
					<display:table requestURI="viralHbv.index.logic" name="${formularios}" export="true" id="formulario" list="${formularios}" pagesize="10" sort="list" class="displaytag">
						...
						<display:column title="Laudo"><a href="viralHbv.formulario.logic?formulario.id=${formulario.id}">Resultado</a></display:column>
						<display:column title=".PDF" media="">
							<c:if test="${formulario.laudoHbv.id != null}"> 
								<a target="_about" href="viralHbv.gerarPdfLaudo.logic?formulario.id=${formulario.id}">.PDF</a>
							</c:if>
						</display:column>
						<display:column title="remover" media="">
							<c:if test="${formulario.laudoHbv.id != null}"> 
								<a href="#" onclick='removerLaudo("${formulario.laudoHbv.id}");'>remover</a>
							</c:if>
						</display:column>
				...
				<script type="text/javascript">
					function removerLaudo(id){
						if(id != null && id != ""){
							window.location = "viralHbv.remove.logic?laudoHbv.id="+ id;
