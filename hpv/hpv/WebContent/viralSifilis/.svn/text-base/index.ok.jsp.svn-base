	...
	<body>
	    <div id="container">
			...
			<div id="contentListar" >
				<h2>Laudos (S&iacute;filis)</h2><!-- campo obrigatorio o h2 no css!!! -->

				<div id='resultadoListar'>
					<display:table requestURI="viralSifilis.index.logic" name="${formularios}" export="true" id="formulario" list="${formularios}" pagesize="10" sort="list" class="displaytag">
						...
						<display:column title="Laudo"><a href="viralSifilis.formulario.logic?formulario.id=${formulario.id}">Resultado</a></display:column>
						<display:column title=".PDF" media="">
							<c:if test="${formulario.laudoSifilis.id != null}"> 
								<a href="viralSifilis.gerarPdfLaudo.logic?formulario.id=${formulario.id}" target="_about">.PDF</a>
							</c:if>
						</display:column>
						
						<display:column title="remover" media="">
							<c:if test="${formulario.laudoSifilis.id != null}"> 
								<a href="#" onclick='removerLaudo("${formulario.laudoSifilis.id}");'>remover</a>
							</c:if>
						</display:column>
				...
				<script type="text/javascript">
					function removerLaudo(id){
						if(id != null && id != ""){
							window.location = "viralSifilis.remove.logic?laudoSifilis.id="+ id;
