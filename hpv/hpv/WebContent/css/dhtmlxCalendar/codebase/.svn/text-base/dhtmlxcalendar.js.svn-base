...
function dhtmlxDblCalendarObject(contId, isAutoDraw, options) {
	...
	this.leftCalendar = new dhtmlxCalendarObject(calLeft, isAutoDraw,
			this.options);
	this.leftCalendar._dblC = this;
	this.leftCalendar.setOnClickHandler(this.doOnCLeftClick);
	this.rightCalendar = new dhtmlxCalendarObject(calRight, isAutoDraw,
			this.options);
...
dhtmlxCalendarObject.prototype.createStructure = function() {
	...
	if (this.con.nodeName == 'INPUT') {
		...
		this.doOnClick = function(date) {
			self.con.value = self
					.getFormatedDate(self.options.dateformat, date);
			self.hide();
			self.con.focus();
			return true
		};
		...
		for ( var i = 0; i < 7; i++) {
			...
			if (this.insensitiveDates) {
				...
				for ( var j = 0; j < this.insensitiveDates.length; j++) {
					var s = this.insensitiveDates[j].indexOf('.') != -1 ? '.'
							: '-';
					var f = (this.insensitiveDates[j].split(s).length == 2 ? '%m'
							+ s + '%d'
							: '%Y' + s + '%m' + s + '%d');
					if (this.getFormatedDate(f, tempDate) == this.insensitiveDates[j]) {
...
dhtmlxCalendarObject.prototype.minimize = function() {
	if (!this.winHeader)
		return;
	this.entBox.style.display = (this.entBox.style.display == 'none') ? 'block'
			: 'none';
	if (!this.minimized)
		this.entObj.className = (this.skinName != "" ? this.style + "_"
				+ this.skinName + " " + this.style + "_" + this.skinName
				: this.style + " " + this.style)
				+ "_minimized";
	else
		this.entObj.className = (this.skinName != "" ? this.style + "_"
				+ this.skinName : this.style);
	if (_isIE) {
...
dhtmlxCalendarObject.prototype.setDate = function(date) {
	...
	if ((date.getFullYear() == this.date.getFullYear())
		...
		var cell = document.getElementById(this.uid + date.getFullYear()
...
dhtmlxCalendarObject.prototype.setSkin = function(newSkin) {
	...
	this.addClass(this.entObj, this.style
			+ (newSkin ? '_' : "")
			+ newSkin
			+ (this.minimized ? " " + this.style
					+ (newSkin ? '_' + newSkin : "") + "_minimized" : ""));
	if (_isIE && this.ifr != undefined) {
		this.ifr.className = this.style + (newSkin ? '_' : "") + newSkin
				+ "_ifr"
	}
};
