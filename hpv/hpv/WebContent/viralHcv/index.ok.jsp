	...
	<body>
	    <div id="container">
			...
			<div id="contentListar" >
				<h2>Laudos (HCV)</h2><!-- campo obrigatorio o h2 no css!!! -->

				<div id='resultadoListar'>
					<display:table requestURI="viralHcv.index.logic" name="${formularios}" export="true" id="formulario" list="${formularios}" pagesize="10" sort="list" class="displaytag">
						...
						<display:column title="Laudo"><a href="viralHcv.formulario.logic?formulario.id=${formulario.id}">Resultado</a></display:column>
						<display:column title=".PDF" media="">
							<c:if test="${formulario.laudoHcv.id != null}"> 
								<a href="viralHcv.gerarPdfLaudo.logic?formulario.id=${formulario.id}");'>.PDF</a>
							</c:if>
						</display:column>						
						<display:column title="remover" media="">
							<c:if test="${formulario.laudoHcv.id != null}">
								<a href="#" onclick='removerLaudo("${formulario.laudoHcv.id}");'>remover</a>
							</c:if>
						</display:column>						
				...
				<script type="text/javascript">
					function removerLaudo(id){
						if(id != null && id != ""){
							window.location = "viralHcv.remove.logic?laudoHcv.id="+ id;
