...
.myPosition {
	...
	<div id="container" style="margin-top: 30px;"><h:form
		...
		<h:commandButton value="Ficha Cadastral"
			rendered="#{sessionHandler.navigation.mostrarVisualizarFuncionario}"
			actionListener="#{relatorioHandler.gerarCadastroFuncionario}">
			<f:attribute name="funcionario"
				value="#{funcionarioHandler.funcionarioAtual}" />
		</h:commandButton>

		<h:panelGroup
			rendered="#{sessionHandler.navigation.mostrarVisualizarFuncionario}">
			<div align="right" style="margin-top: 20px;"><h:commandButton
				value="Voltar" action="pesquisarFuncionario" /> <h:commandButton
				value="Editar"
				action="#{sessionHandler.navigation.editarFuncionario}">
				<f:setPropertyActionListener value="#{sessionHandler.usuario}"
					target="#{funcionarioHandler.user}" />
			</h:commandButton></div>
		</h:panelGroup>

		<h:panelGroup
			rendered="#{sessionHandler.navigation.mostrarEditarFuncionario}">
			<div align="right" style="margin-top: 20px;"><h:commandButton
				value="Cancelar" action="#{funcionarioHandler.refresh}"
				immediate="true" /> <h:commandButton value="Salvar"
				action="#{funcionarioHandler.salvarFuncionario}">
				<f:setPropertyActionListener value="#{sessionHandler.usuario}"
					target="#{funcionarioHandler.user}" />
			</h:commandButton></div>
		</h:panelGroup>
		...
		<f:subview id="editarFuncionario"
			rendered="#{sessionHandler.navigation.mostrarEditarFuncionario}">
			<jsp:include page="/funcionario/editarFuncionario.jsp"></jsp:include>
		</f:subview>

		<f:subview id="visualizarFuncionario"
			rendered="#{sessionHandler.navigation.mostrarVisualizarFuncionario}">
			<jsp:include page="/funcionario/visualizarFuncionario.jsp"></jsp:include>
		</f:subview>
		...
		<div align="center"><h:panelGrid columns="4">
			...
			<h:outputLabel style="font-weight:bold;"
				value="#{funcionarioHandler.funcionarioAtual.usuarioAtualizou.login}"
				rendered="#{funcionarioHandler.funcionarioAtual.usuarioAtualizou != null}"></h:outputLabel>
			...
			<h:outputText
				value="#{funcionarioHandler.funcionarioAtual.dataAtualizacao}"
	...
	<f:subview id="criarCracha">
		<jsp:include page="/funcionario/lookup/criarCracha.jsp"></jsp:include>
	</f:subview>

	<f:subview id="lookupFuncao">
		<jsp:include page="/funcionario/lookup/lookupFuncao.jsp"></jsp:include>
	</f:subview>
