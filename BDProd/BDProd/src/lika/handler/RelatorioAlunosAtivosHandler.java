...
@Controller("relatorioAlunosAtivosHandler")
@Scope("session")
public class RelatorioAlunosAtivosHandler {
		...
		try {
			...
			parametros
					.put(
							"caminhoSub",
							servletContext
									.getResourceAsStream("/relatorios/SubRelatorioAlunosAtivos.jasper"));
	...
	public void gerarDadosAluno(ActionEvent event) {
		...
		try {
			...
			try {
				...
				InputStream reportStream = context.getExternalContext()
						.getResourceAsStream(
								"/relatorios/RelatorioAlunosAtivos.jasper");
