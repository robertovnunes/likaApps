...
<f:view>
	...
	<div id="page"><h:form>
		...
		<ul>
			...
			<h2>Opções</h2>
			<ul>
				<li><h:commandLink value="Novo Bolsista"
					action="#{sessionHandler.navigation.novoBolsista}"
					actionListener="#{bolsistaProjetoHandler.novoBolsistaProjeto}" /></li>
			</ul>
			</li>
		</ul>
		</div>
		...
		<div id="content"><a4j:region id="regiao">
			...
			<h:panelGroup id="campo">
				<h:selectOneMenu value="#{bolsistaProjetoHandler.parametroConsulta}"
					rendered="#{bolsistaProjetoHandler.mostrarPesquisaSexo}">
					<f:selectItem itemValue="M" itemLabel="Masculino" />
					<f:selectItem itemValue="F" itemLabel="Feminino" />
				</h:selectOneMenu>

				<h:inputText id="textBuscaNome" style="width: 400px"
					value="#{bolsistaProjetoHandler.parametroConsulta}"
					rendered="#{bolsistaProjetoHandler.mostrarPesquisaNome}" />

				<h:inputText id="textBuscaCPF" style="width: 400px"
					value="#{bolsistaProjetoHandler.parametroConsulta}"
					rendered="#{bolsistaProjetoHandler.mostrarPesquisaCPF}"
					onkeypress="mascara(this,cpf)" maxlength="14" />

				<a4j:commandButton id="botaoBuscar" value="Buscar"
					action="#{bolsistaProjetoHandler.consultar}" reRender="panel"
					onclick="this.disabled=true" oncomplete="this.disabled=false" />


			</h:panelGroup> <h:selectOneRadio value="#{bolsistaProjetoHandler.tipoSelecionado}">
				<f:selectItems value="#{bolsistaProjetoHandler.tiposDePesquisas}" />
				<a4j:support reRender="campo" event="onclick"
					action="#{bolsistaProjetoHandler.limparConsulta}" ajaxSingle="true"
					status="a" />
			</h:selectOneRadio></div>

			<a4j:outputPanel id="panel">

				<h:panelGroup rendered="#{empty bolsistaProjetoHandler.bolsistas }">
					<div id="nenhum" align="center" style="color: red;"><h:outputText
						value="Nenhum Bolsista econtrado. " /></div>
				</h:panelGroup>
				...
				<h:panelGroup
					rendered="#{not empty bolsistaProjetoHandler.bolsistas }">
					<div class="titulo" style="margin-left: 15px; margin-bottom: 15px">
					<h:outputText value="Bolsistas" /></div>

					<div style="margin-left: 10px;"><rich:dataTable
						id="tabelaResultado" value="#{bolsistaProjetoHandler.bolsistas}"
						...
						<rich:column id="opcoes">
							...
							<h:commandLink
								action="#{sessionHandler.navigation.visualizarBolsista}"
								actionListener="#{bolsistaProjetoHandler.carregarBolsistaProjeto}">
								<f:attribute name="idBolsista" value="#{func.idPessoa}" />


								<h:graphicImage value="../images/editar4.png"
									style="border:none;" title="Editar Bolsista Projeto"
									alt="Editar Bolsista Projeto" width="17" height="17"></h:graphicImage>
