...
<div id="content">
	...
	<div id="inner">
		...
		<div id="participantes" align="center">
			<c:forEach var="alun" items="${equipe.alunos}">
				${alun.nome}, 
			</c:forEach>
			
			${problema.equipe.tutor.nome}<br/><br/>	 
		</div>
		<div align="center">
		...
		<div id="miolo">
			...
			<b>Palavras-Chave: </b><c:if test="${not empty problema.palavrasChave}">${fs:listarPalavras(problema.palavrasChave)}</c:if>
			...
			<c:forEach var="materialGrafico" items="${problema.materiaisGrafico}">
				<p align="center">
					<img src="relatorio.mostraImagem.logic?materialGrafico.idMaterialGrafico=${materialGrafico.idMaterialGrafico}"/><br/>
