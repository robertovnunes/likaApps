		...
		<%@ include file="menuProfessor.jsp" %>
			...
			<display:table name="${problemas}" id="problema" cellpadding="1" cellspacing="1">
				...
				<display:column title="A&ccedil;&otilde;es"	style="width: 190px; text-align: center;">
					<a href="professor.editarProblema.logic?problema.idProblema=${problema.idProblema}"><img
						src="images/editar.gif" /></a>
					<a href="#"  target="_self" onclick="conf('professor.removerProblema.logic?problema.idProblema=${problema.idProblema}', '${problema.titulo}');">	
					<img src="images/remover.gif" /></a>
					<c:if test="${problema.aberto == 0}">
						<a href="professor.formularioFecharProblema.logic?problema.idProblema=${problema.idProblema}">
							<img src="images/fechar.gif" title="Fechar"/>
						</a>	
					</c:if>
					<c:if test="${problema.aberto == 1}">
						<a href="professor.formularioFecharProblema.logic?problema.idProblema=${problema.idProblema}">
							<img src="images/abrir.gif" title="Abrir"/>
						</a>
					</c:if> 
					<a href="avaliacao.notasEquipe.logic?equipe.id=${problema.equipe.id}">
						<img src="images/avaliar.png"/>
					</a>
				</display:column>
			</display:table>
		</div>
