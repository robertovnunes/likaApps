...
class RepositorioIS
{
	...
	{
		mysql_query("INSERT INTO informante (idAtend, nome, sexo, confia, ender, num, compl, bairro, cidade, uf, cep, idParent, 
			idEsc, obs, ass) VALUES ('". $idAtend ."','" . $info->getNome() . "','" . $info->getSexo(). "','a','" . 
			$info->getEndereco() . "',"  . $info->getNumero() . ",'" . $info->getComplemento() . "','" . $info->getBairro() . "','" . $info->getCidade() . 
	...
	function alterarInformante($info, $idAtend)
	{
		mysql_query("UPDATE informante SET nome = '" . $info->getNome() . "', cidade = '" . $info->getCidade() . "', uf='" . 
			...
			$info->getEndereco() . "', num = '" . $info->getNumero() . "', compl = '" . $info->getComplemento() . "', confia='a', obs = '" . $info->getObs() . "', bairro = '" .
			$info->getBairro() . "', cep = '" . $info->getCEP() . "', sexo='" . $info->getSexo() . "', sav = '1' WHERE idAtend = '" . $idAtend . "'") or die("ERRO no comando SQL:" . mysql_error());
	}
