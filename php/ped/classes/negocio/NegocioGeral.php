...
class NegocioGeral{
	...
	function msgData($data){
		...
		if (checkdate(substr($data, 3, 2), substr($data, 0, 2), substr($data, 6, 4))){
			...
			else if ($this->calculoIdade(substr($data,0,2),substr($data,3,2),substr($data, 6, 4)) > 21)
				return "Insira um paciente com no máximo 21 anos de idade";
	...
	function validaEmail($email){
		...
		return preg_match("/^([A-Z_a-z])+.+@([a-zA-Z])+/", $email);
	}
	...
	function checkPOST($id, $value, $field) {
		...
		if ($value == "NA" && ($field == "" || $field == NULL) && (!isset($_POST[$id]) || $_POST[$id] == "" || $_POST[$id] == NULL))
			return "checked='true'";
	}
...
//			return "disabled='true'";
		if ((isset($_POST[$id]) and $_POST[$id] == $value) || $field == $value)
			return "";
		return "disabled='true'";
	}
	...
	function checkDisable3($id, $field, $value, $value2, $value3) {
		if ((isset($_POST[$id]) and $_POST[$id]==$value) || $field==$value || (isset($_POST[$id]) and $_POST[$id]==$value2) || $field==$value2 || (isset($_POST[$id]) and $_POST[$id]==$value3) || $field==$value3)
			return "";
		return "disabled='true'";
