...
class NegocioPaciente {
	...
	public function existePaciente($paciente) {
 	...
 	public function cadastrar($paciente, $forcar, $login) {
        ...
        if (!$invalido && $this->validaCPF($paciente->getCPF())) {
			$idPront = $this->existePaciente($paciente);
			if (!$idPront || $forcar){
				$idAtendimento = $this->repPac->cadastrar($paciente, $login);
			} else {		
				throw new ExistePaciente($idPront);
			}
        } else {
    ...
    function parametrosInvalidos($nome, $data, $sexo, $mae, $endereco, $bairro, $cidade, $estado) {
