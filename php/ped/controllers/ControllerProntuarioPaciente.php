...
if((isset($_POST['vhip'])) and ($_POST['vhip'] == "Ver Hipótese")){  //Não tem mais, foi trocado por confimar hipótese
	$hipotese = $fachada->consultarHD($_POST['idhipot']);
	$txthip = "Atualizar Hipótese";
	$flag = 1;
}

if((isset($_POST['chip'])) and ($_POST['chip'] == "Confirmar Hipótese")){
	$hipConfirmada = $fachada->confirmarHip($_POST['idhipot'], $user->getID());
	$flag = 1;
}

if((isset($_POST['nhip'])) and ($_POST['nhip'] == "Adicionar")){ //Adicionar Hipótese
	try{
...
if((isset($_POST['nhip'])) and ($_POST['nhip'] == "Atualizar Hipótese")){
	try{
...
if (!$txthip)
	$txthip = "Adicionar"; //Adicionar Hipótese

if((isset($_POST['rhip'])) and ($_POST['rhip'] == "Remover Hipótese")){
	$fachada->removerHD($_POST['idhipot']);
	$flag = 1;
}

if ((isset($_POST['salvar']) and $_POST['salvar'] == "Salvar") || $assinar){
	$flag = 1;
	...
	switch ($aba){
		...
		case "informante":
			try {
				...
				$informante = new Informante($_POST['nome'], isset($_POST['sexo']) ? $_POST['sexo'] : '', $_POST['ende'], $_POST['num'],
		...
		case "geral":
			...
			if (!$ass){
				try {	
					$fachada->inserirGeralIS(isset($_POST['febre']) ? $_POST['febre'] : '', $_POST['altpeso'], $_POST['atividade'], 
		...
		case "pele":
			...
			if (!$ass){
				try {
					$fachada->inserirPeleIS($_POST['rash'], $_POST['ictericia'], isset($_POST['infecrep']) ? $_POST['infecrep'] : '', 
						$_POST['obspele'], $atend);
					$msgi = 1;
				} catch (CamposInvalidos $e) {
					$msge = 1;
		...
		case "olhos":
			...
			if (!$ass){
				try {
					$fachada->inserirOlhosIS(isset($_POST['fotofobia']) ? $_POST['fotofobia'] : '',
						isset($_POST['lacrim']) ? $_POST['lacrim'] : '', isset($_POST['edemapalp']) ? $_POST['edemapalp'] : '',
						isset($_POST['secrecaool']) ? $_POST['secrecaool'] : '', $_POST['dorolho'], $_POST['acuidvis'], $_POST['obsolho'],
						$atend);
					$msgi = 1;
				} catch (CamposInvalidos $e) {
					$msge = 1;
				...
				if (!$msge && $assinar)
					$fachada->assinarIS(1, $atend, $user->getID());
			}else {
				try {
					$fachada->adendoIS($_POST['adendoNovo'], $atend, $user->getID());
		...
		case "ouvidos":
			...
			if (!$ass){
				try {
					$fachada->inserirOuvidosIS(isset($_POST['infecfreq']) ? $_POST['infecfreq'] : '',
						isset($_POST['secrecaoou']) ? $_POST['secrecaoou'] : '', $_POST['dorouvido'],
						$_POST['acuidaud'], $_POST['obsouvido'], $atend);
					$msgi = 1;
				} catch (CamposInvalidos $e) {
					$msge = 1;
				}
		...
		case "exame_apresp":		
			...
			if (!$ass){
				try {
					$fachada->inserirRespEX(
						isset($_POST['formtorax']) ? $_POST['formtorax'] : '', isset($_POST['simtorax']) ? $_POST['simtorax'] : '',
						isset($_POST['contornos']) ? $_POST['contornos'] : '', isset($_POST['proemi']) ? $_POST['proemi'] : '',
						isset($_POST['massasanorm']) ? $_POST['massasanorm'] : '', isset($_POST['mamas']) ? $_POST['mamas'] : '',
						isset($_POST['tresp']) ? $_POST['tresp'] : '', isset($_POST['tirinterc']) ? $_POST['tirinterc'] : '',
						isset($_POST['freqresp']) ? $_POST['freqresp'] : '', isset($_POST['fremtv']) ? $_POST['fremtv'] : '',
						isset($_POST['frembronq']) ? $_POST['frembronq'] : '', isset($_POST['frempleur']) ? $_POST['frempleur'] : '',
						isset($_POST['submacicez']) ? $_POST['submacicez'] : '', isset($_POST['macicez']) ? $_POST['macicez'] : '',
						isset($_POST['timpanico']) ? $_POST['timpanico'] : '', isset($_POST['murmvesic']) ? $_POST['murmvesic'] : '',
						isset($_POST['estertores']) ? $_POST['estertores'] : '', isset($_POST['estridores']) ? $_POST['estridores'] : '',
						isset($_POST['sibilos']) ? $_POST['sibilos'] : '', isset($_POST['atritopl']) ? $_POST['atritopl'] : '',
						isset($_POST['obsresp']) ? $_POST['obsresp'] : '', $atend);
					$msgi = 1;
				} catch (CamposInvalidos $e) {
					$msge = 1;
				}
		...
		case "exame_medind":
			...
			if (!$ass){
				try {
					$fachada->inserirMedindEX($_POST['pesomed'],
						isset($_POST['pesomedin']) ? $_POST['pesomedin'] : '', $_POST['alturamed'],
						isset($_POST['alturamedin']) ? $_POST['alturamedin'] : '',
						$_POST['permcef'], $_POST['permtora'], $_POST['permab'], 
		...
		case "conduta":
			$linha = $fachada->consultarCD($idHipotese);
			$ass = $fachada->checkAssinado($linha->ass);
			if (!$ass){
				try {
					$fachada->alterarConduta($_POST['conduta'], $idHipotese, $assinar, $user->getID());
					$msgi = 1;
				} catch (CamposInvalidos $e) {
					$msge = 1;
				}
				...
				try {
					$fachada->adendoCD($_POST['adendoNovo'], $idHipotese, $user->getID());
					header('Location:prontuario_paciente.php?at='.$atend.'&aba=conduta&hd='.$idHipotese.'&flag=1');
				} catch (CamposInvalidos $e) {
					$errad = 1;
				}
			}
...
if($aba == "conduta"){
	...
	$ass = $fachada->checkAssinado($linha->ass);
	if ($ass) $adendoad = $fachada->getAdendoCD($idHipotese);
	$dadendo = $fachada->checkAssinado(!$linha->ass);
	$jsabas = 'onclick="verificar();"';
}
...
if(isset($linha) and is_object($linha)){
...
switch ($aba) { 
	...
	case "geral":
		...
		$altpeso1 = $fachada->checkOptionIS("altpeso", "GP", $lger->altpeso);
		$altpeso2 = $fachada->checkOptionIS("altpeso", "PP", $lger->altpeso);
		$altpeso3 = $fachada->checkOptionIS("altpeso", "NE", $lger->altpeso);
		$altpeso4 = $fachada->checkOptionIS("altpeso", "NA", $lger->altpeso);
		$apetite1 = $fachada->checkOptionIS("apetite", "F", $lger->apetite);
		$apetite2 = $fachada->checkOptionIS("apetite", "E", $lger->apetite);
		$apetite3 = $fachada->checkOptionIS("apetite", "N", $lger->apetite);
		$apetite4 = $fachada->checkOptionIS("apetite", "NA", $lger->apetite);
		$atividade1 = $fachada->checkOptionIS("atividade", "A", $lger->atividade);
		$atividade2 = $fachada->checkOptionIS("atividade", "Q", $lger->atividade);
		$atividade3 = $fachada->checkOptionIS("atividade", "NA", $lger->atividade);
		...
		$obsgeral = $fachada->printOBS($lger->obs, (isset($_POST['obsgeral']) ? $_POST['obsgeral'] : ''));
    ...
    case "pele":
		...
		$rash1 = $fachada->checkOptionIS("rash", "E", $lpele->rash);
		$rash2 = $fachada->checkOptionIS("rash", "M", $lpele->rash);
		$rash3 = $fachada->checkOptionIS("rash", "A", $lpele->rash);
		$rash4 = $fachada->checkOptionIS("rash", "NA", $lpele->rash);
		$ictericia1 = $fachada->checkOptionIS("ictericia", "A", $lpele->ictericia);
		$ictericia2 = $fachada->checkOptionIS("ictericia", "NA", $lpele->ictericia);
		...
		$obspele = $fachada->printOBS($lpele->obs, (isset($_POST['obspele']) ? $_POST['obspele'] : ''));
    ...
    case "olhos":
		...
		$dorolho1 = $fachada->checkOptionIS("dorolho", "OE", $lolh->dorolho);
		$dorolho2 = $fachada->checkOptionIS("dorolho", "OD", $lolh->dorolho);
		$dorolho3 = $fachada->checkOptionIS("dorolho", "AO", $lolh->dorolho);
		$dorolho4 = $fachada->checkOptionIS("dorolho", "N", $lolh->dorolho);
		$dorolho5 = $fachada->checkOptionIS("dorolho", "NA", $lolh->dorolho);
		$acuidvis1 = $fachada->checkOptionIS("acuidvis", "TN", $lolh->acuidvis);
		$acuidvis2 = $fachada->checkOptionIS("acuidvis", "TA", $lolh->acuidvis);
		$acuidvis3 = $fachada->checkOptionIS("acuidvis", "S", $lolh->acuidvis);
		$acuidvis4 = $fachada->checkOptionIS("acuidvis", "N", $lolh->acuidvis);
		$acuidvis5 = $fachada->checkOptionIS("acuidvis", "C", $lolh->acuidvis);
		$acuidvis6 = $fachada->checkOptionIS("acuidvis", "NA", $lolh->acuidvis);
		$obsolho = $fachada->printOBS($lolh->obs, (isset($_POST['obsolho']) ? $_POST['obsolho'] : ''));
    ...
    case "ouvidos":
		...
		$dorouv1 = $fachada->checkOptionIS("dorouvido", "OE", $louv->dorouvido);
		$dorouv2 = $fachada->checkOptionIS("dorouvido", "OD", $louv->dorouvido);
		$dorouv3 = $fachada->checkOptionIS("dorouvido", "AO", $louv->dorouvido);
		$dorouv4 = $fachada->checkOptionIS("dorouvido", "N", $louv->dorouvido);
		$dorouv5 = $fachada->checkOptionIS("dorouvido", "NA", $louv->dorouvido);
		$acuidaud1 = $fachada->checkOptionIS("acuidaud", "TN", $louv->acuidaud);
		$acuidaud2 = $fachada->checkOptionIS("acuidaud", "TA", $louv->acuidaud);
		$acuidaud3 = $fachada->checkOptionIS("acuidaud", "S", $louv->acuidaud);
		$acuidaud4 = $fachada->checkOptionIS("acuidaud", "N", $louv->acuidaud);
		$acuidaud5 = $fachada->checkOptionIS("acuidaud", "NA", $louv->acuidaud);
		$obsouvido = $fachada->printOBS($louv->obs, (isset($_POST['obsouvido']) ? $_POST['obsouvido'] : ''));
    ...
    case "respiratorio":
		...
		$corrnasal1 = $fachada->checkOptionIS("corrnasal", "CL", $lrsp->corrnasal);
		$corrnasal2 = $fachada->checkOptionIS("corrnasal", "ES", $lrsp->corrnasal);
		$corrnasal3 = $fachada->checkOptionIS("corrnasal", "A", $lrsp->corrnasal);
		$corrnasal4 = $fachada->checkOptionIS("corrnasal", "NA", $lrsp->corrnasal);
		...
		$ttosse1 = $fachada->checkOptionIS("ttosse", "P", $lrsp->ttosse);
		$ttosse2 = $fachada->checkOptionIS("ttosse", "S", $lrsp->ttosse);
		$ttosse3 = $fachada->checkOptionIS("ttosse", "NA", $lrsp->ttosse);
		$dtosse1 = $fachada->checkOptionIS("dtosse", "-30",$lrsp->dtosse);
		$dtosse2 = $fachada->checkOptionIS("dtosse", "+30",$lrsp->dtosse);
		$dtosse3 = $fachada->checkOptionIS("dtosse", "NA",$lrsp->dtosse);
		$ptosse1 = $fachada->checkOptionIS("ptosse", "N", $lrsp->ptosse);
		$ptosse2 = $fachada->checkOptionIS("ptosse", "M", $lrsp->ptosse);
		$ptosse3 = $fachada->checkOptionIS("ptosse", "NA", $lrsp->ptosse);
		...
		$obsresp = $fachada->printOBS($lrsp->obs, (isset($_POST['obsresp']) ? $_POST['obsresp'] : ''));
    ...
    case "cardiovascular":
		...
		$cianose1 = $fachada->checkOptionIS("cianose", "L", $lapcv->cianose);
		$cianose2 = $fachada->checkOptionIS("cianose", "G", $lapcv->cianose);
		$cianose3 = $fachada->checkOptionIS("cianose", "A", $lapcv->cianose);
		$cianose4 = $fachada->checkOptionIS("cianose", "NA", $lapcv->cianose);
		...
		$obscv = $fachada->printOBS($lapcv->obs, (isset($_POST['obscv']) ? $_POST['obscv'] : ''));
    ...
    case "gastrointestinal":
		...
		$vomitos1 = $fachada->checkOptionIS("vomitos", "A", $lgi->vomitos);
		$vomitos2 = $fachada->checkOptionIS("vomitos", "AA", $lgi->vomitos);
		$vomitos3 = $fachada->checkOptionIS("vomitos", "SC", $lgi->vomitos);
		$vomitos4 = $fachada->checkOptionIS("vomitos", "N", $lgi->vomitos);
		$vomitos5 = $fachada->checkOptionIS("vomitos", "NA", $lgi->vomitos);
		...
		$aspfezes1 = $fachada->checkOptionIS("aspfezes", "N", $lgi->aspfezes);
		$aspfezes2 = $fachada->checkOptionIS("aspfezes", "B", $lgi->aspfezes);
		$aspfezes3 = $fachada->checkOptionIS("aspfezes", "E", $lgi->aspfezes);
		$aspfezes4 = $fachada->checkOptionIS("aspfezes", "L", $lgi->aspfezes);
		$aspfezes5 = $fachada->checkOptionIS("aspfezes", "P", $lgi->aspfezes);
		$aspfezes6 = $fachada->checkOptionIS("aspfezes", "NA", $lgi->aspfezes);
		if (isset($errgi) && (!$_POST['nfezes'] || !is_numeric($_POST['nfezes']))) 
			$lblnfez = 'class="erro"';
		$nfezes = $fachada->printCampo($lgi->nfezes, (isset($_POST['nfezes']) ? $_POST['nfezes'] : ''));
		$obsgi = $fachada->printOBS($lgi->obs, (isset($_POST['obsgi']) ? $_POST['obsgi'] : ''));
    ...
    case "genitourinario":
		...
		$qtdur1 = $fachada->checkOptionIS("qtdurina", "N", $lgu->qtdurina);	
		$qtdur2 = $fachada->checkOptionIS("qtdurina", "A", $lgu->qtdurina);	
		$qtdur3 = $fachada->checkOptionIS("qtdurina", "E", $lgu->qtdurina);	
		$qtdur4 = $fachada->checkOptionIS("qtdurina", "NA", $lgu->qtdurina);	
		$odorur1 = $fachada->checkOptionIS("odorurina", "C", $lgu->odorurina);
		$odorur2 = $fachada->checkOptionIS("odorurina", "F", $lgu->odorurina);
		$odorur3 = $fachada->checkOptionIS("odorurina", "NA", $lgu->odorurina);
		$jatour1 = $fachada->checkOptionIS("jatourina", "C", $lgu->jatourina);
		$jatour2 = $fachada->checkOptionIS("jatourina", "P", $lgu->jatourina);
		$jatour3 = $fachada->checkOptionIS("jatourina", "NA", $lgu->jatourina);
		$frequr1 = $fachada->checkOptionIS("frequrina", "N", $lgu->frequrina);
		$frequr2 = $fachada->checkOptionIS("frequrina", "MH", $lgu->frequrina);
		$frequr3 = $fachada->checkOptionIS("frequrina", "NA", $lgu->frequrina);
		$corur1 = $fachada->checkOptionIS("corurina", "C", $lgu->corurina);
		$corur2 = $fachada->checkOptionIS("corurina", "E", $lgu->corurina);
		$corur3 = $fachada->checkOptionIS("corurina", "NA", $lgu->corurina);
		...
		$ccuret1 = $fachada->checkOptionIS("corcorruret", "C", $lgu->corcorruret);
		$ccuret2 = $fachada->checkOptionIS("corcorruret", "L", $lgu->corcorruret);
		$ccuret3 = $fachada->checkOptionIS("corcorruret", "NA", $lgu->corcorruret);
		$ocuret1 = $fachada->checkOptionIS("odorcorruret", "I", $lgu->odorcorruret);
		$ocuret2 = $fachada->checkOptionIS("odorcorruret", "F", $lgu->odorcorruret);
		$ocuret3 = $fachada->checkOptionIS("odorcorruret", "NA", $lgu->odorcorruret);
		...
		if ($paciente->getSexo() == "M"){
			...
			$pubarcah1 = $fachada->checkOptionIS("pubarcah", "-8", $lsm->pubarcah);
			$pubarcah2 = $fachada->checkOptionIS("pubarcah", "814", $lsm->pubarcah);
			$pubarcah3 = $fachada->checkOptionIS("pubarcah", "+14", $lsm->pubarcah);
			$pubarcah4 = $fachada->checkOptionIS("pubarcah", "A", $lsm->pubarcah);
			$pubarcah5 = $fachada->checkOptionIS("pubarcah", "NA", $lsm->pubarcah);
			$voltest1 = $fachada->checkOptionIS("voltest", "N", $lsm->voltest);
			$voltest2 = $fachada->checkOptionIS("voltest", "MA", $lsm->voltest);
			$voltest3 = $fachada->checkOptionIS("voltest", "ME", $lsm->voltest);
			$voltest4 = $fachada->checkOptionIS("voltest", "NA", $lsm->voltest);
			...
			$tampenis1 = $fachada->checkOptionIS("tampenis", "N", $lsm->tampenis);
			$tampenis2 = $fachada->checkOptionIS("tampenis", "MA", $lsm->tampenis);
			$tampenis3 = $fachada->checkOptionIS("tampenis", "ME", $lsm->tampenis);
			$tampenis4 = $fachada->checkOptionIS("tampenis", "NA", $lsm->tampenis);
			...
			$corcorrvag1 = $fachada->checkOptionIS("corcorrvag", "C", $lsf->corcorrvag);
			$corcorrvag2 = $fachada->checkOptionIS("corcorrvag", "L", $lsf->corcorrvag);
			$corcorrvag3 = $fachada->checkOptionIS("corcorrvag", "E", $lsf->corcorrvag);
			$corcorrvag4 = $fachada->checkOptionIS("corcorrvag", "NA", $lsf->corcorrvag);
			$odorcorrvag1 = $fachada->checkOptionIS("odorcorrvag", "I", $lsf->odorcorrvag);
			$odorcorrvag2 = $fachada->checkOptionIS("odorcorrvag", "F", $lsf->odorcorrvag);
			$odorcorrvag3 = $fachada->checkOptionIS("odorcorrvag", "NA", $lsf->odorcorrvag);
			...
			$telarca1 = $fachada->checkOptionIS("telarca", "-8", $lsf->telarca);
			$telarca2 = $fachada->checkOptionIS("telarca", "813", $lsf->telarca);
			$telarca3 = $fachada->checkOptionIS("telarca", "+13", $lsf->telarca);
			$telarca4 = $fachada->checkOptionIS("telarca", "A", $lsf->telarca);
			$telarca5 = $fachada->checkOptionIS("telarca", "NA", $lsf->telarca);
			$pubarcam1 = $fachada->checkOptionIS("pubarcam", "-6", $lsf->pubarcam);
			$pubarcam2 = $fachada->checkOptionIS("pubarcam", "613", $lsf->pubarcam);
			$pubarcam3 = $fachada->checkOptionIS("pubarcam", "+13", $lsf->pubarcam);
			$pubarcam4 = $fachada->checkOptionIS("pubarcam", "A", $lsf->pubarcam);
			$pubarcam5 = $fachada->checkOptionIS("pubarcam", "NA", $lsf->pubarcam);
			...
			$menarca1 = $fachada->checkOptionIS("menarca", "-8", $lsf->menarca);	
			$menarca2 = $fachada->checkOptionIS("menarca", "816", $lsf->menarca);
			$menarca3 = $fachada->checkOptionIS("menarca", "+16", $lsf->menarca);
			$menarca4 = $fachada->checkOptionIS("menarca", "NA", $lsf->menarca);
			$regular1 = $fachada->checkOptionIS("regularidade", "-28", $lsf->regularidade);
			$regular2 = $fachada->checkOptionIS("regularidade", "28", $lsf->regularidade);
			$regular3 = $fachada->checkOptionIS("regularidade", "+28", $lsf->regularidade);
			$regular4 = $fachada->checkOptionIS("regularidade", "NA", $lsf->regularidade);
			$fluxo1 = $fachada->checkOptionIS("fluxo", "N", $lsf->fluxo);
			$fluxo2 = $fachada->checkOptionIS("fluxo", "A", $lsf->fluxo);
			$fluxo3 = $fachada->checkOptionIS("fluxo", "D", $lsf->fluxo);
			$fluxo4 = $fachada->checkOptionIS("fluxo", "NA", $lsf->fluxo);
		}
		$obsgu = $fachada->printOBS($lgu->obs, (isset($_POST['obsgu']) ? $_POST['obsgu'] : ''));
    ...
    case "musculoesqueletico":
		...
		$obsme = $fachada->printOBS($lme->obs, (isset($_POST['obsme']) ? $_POST['obsme'] : ''));
    ...
    case "nervoso":
		...
		$convulsoes1 = $fachada->checkOptionIS("convulsoes", "F", $lnerv->convulsoes);
		$convulsoes2 = $fachada->checkOptionIS("convulsoes", "AF", $lnerv->convulsoes);
		$convulsoes3 = $fachada->checkOptionIS("convulsoes", "A", $lnerv->convulsoes);
		$convulsoes4 = $fachada->checkOptionIS("convulsoes", "NA", $lnerv->convulsoes);
		$dconvulsoes = $fachada->checkDisable2("convulsoes", $lnerv->convulsoes, "F", "AF");
		$conv11 = $fachada->checkOptionIS("conv1", "T", $lnerv->conv1);
		$conv12 = $fachada->checkOptionIS("conv1", "TC", $lnerv->conv1);
		$conv13 = $fachada->checkOptionIS("conv1", "NA", $lnerv->conv1);
		$conv21 = $fachada->checkOptionIS("conv2", "L", $lnerv->conv2);
		$conv22 = $fachada->checkOptionIS("conv2", "G", $lnerv->conv2);
		$conv23 = $fachada->checkOptionIS("conv2", "NA", $lnerv->conv2);
		...
		$obsnerv = $fachada->printOBS($lnerv->obs, (isset($_POST['obsnerv']) ? $_POST['obsnerv'] : ''));
    ...
    case "exame_inspec":
		...
		$obsins = $fachada->printOBS($leins->obs, (isset($_POST['obsins']) ? $_POST['obsins'] : ''));
    ...
    case "exame_pele":
		...
		$obspele = $fachada->printOBS($lepele->obs, (isset($_POST['obsepele']) ? $_POST['obsepele'] : ''));
    ...
    case "exame_gang":
		...
		$obsegang = $fachada->printOBS($legang->obs, (isset($_POST['obsegang']) ? $_POST['obsegang'] : ''));
    ...
    case "exame_estpub":
		...
		$obsantrop = $fachada->printOBS($lant->obs, (isset($_POST['obsantrop']) ? $_POST['obsantrop'] : ''));
    ...
    case "exame_desneuro":
		...
		$obsdes = $fachada->printOBS($ldes->obs, (isset($_POST['obsdes']) ? $_POST['obsdes'] : ''));
    ...
    case "exame_medind":
		...
		$pesomed1 = $fachada->checkOptionIS("pesomed", "NA", $medind->peso);
		$pesomed2 = $fachada->checkOptionIS("pesomed", "AV", $medind->peso);
		$pesomedin = $fachada->printOBS($medind->pesoin, (isset($_POST['pesomedin']) ? $_POST['pesomedin'] : ''));
		if ($msge && $_POST['pesomed'] == "AV" && !is_numeric($_POST['pesomedin']))
			$lblpeso = "class='erro'";
		$dpesomed = $fachada->checkDisable("pesomed", $medind->peso, "AV");
		$alturamed1 = $fachada->checkOptionIS("alturamed", "NA", $medind->altura);
		$alturamed2 = $fachada->checkOptionIS("alturamed", "AV", $medind->altura);
		$dalturamed = $fachada->checkDisable("alturamed", $medind->altura, "AV");
		$alturamedin = $fachada->printOBS($medind->alturain, (isset($_POST['alturamedin']) ? $_POST['alturamedin'] : ''));
		if ($msge && $_POST['alturamed'] == "AV" && !is_numeric($_POST['alturamedin']))
			$lblaltmed = "class='erro'";
		$permcef = $fachada->printOBS($medind->permcef, (isset($_POST['permcef']) ? $_POST['permcef'] : ''));
		if ($msge && !is_numeric($_POST['permcef']))
			$lblpcef = "class='erro'";
		$permtora = $fachada->printOBS($medind->permtora, (isset($_POST['permtora']) ? $_POST['permtora'] : ''));
		if ($msge && !is_numeric($_POST['permtora']))
			$lblptor = "class='erro'";
		$permab = $fachada->printOBS($medind->permab, (isset($_POST['permab']) ? $_POST['permab'] : ''));
		if ($msge && !is_numeric($_POST['permab']))
			$lblpmab = "class='erro'";
		$pregatric = $fachada->printOBS($medind->pregatric, (isset($_POST['pregatric']) ? $_POST['pregatric'] : ''));
		if ($msge && !is_numeric($_POST['pregatric']))
			$lblptric = "class='erro'";
		$pregasubesc = $fachada->printOBS($medind->pregasubesc, (isset($_POST['pregasubesc']) ? $_POST['pregasubesc'] : ''));
    ...
    case "exame_cranio":
		...
		$obscran = $fachada->printOBS($lcr->obs, (isset($_POST['obscran']) ? $_POST['obscran'] : ''));
    ...
    case "exame_olhos":
		...
		$obsolho = $fachada->printOBS($leolh->obs, (isset($_POST['obsolho']) ? $_POST['obsolho'] : ''));
    ...
    case "exame_sistotor":
		...
		$secrnariz0 = $fachada->checkOption("secrnariz", "N", $leot->secrnariz);
		...
		$obsotor = $fachada->printOBS($leot->obs, (isset($_POST['obsotor']) ? $_POST['obsotor'] : ''));
    ...
    case "exame_pescoco":
		...
		$obspesc = $fachada->printOBS($lepes->obs, (isset($_POST['obspesc']) ? $_POST['obspesc'] : '')); 
    ...
    case "exame_cardio":
		...
		$presartin = $fachada->printOBS($lecdvs->pressaoartin, (isset($_POST['pressaoartin']) ? $_POST['pressaoartin'] : ''));
		if ($msge && $_POST['pressaoart'] == "AV" && !preg_match("/^([0-2][0-9])[\/]([0-9])$/", $pressaoartin) )
			$lblpart = "class='erro'";
		...
		$frqcrdin = $fachada->printOBS($lecdvs->freqcardin, (isset($_POST['freqcardin']) ? $_POST['freqcardin'] : ''));
		if ($msge && $_POST['freqcard'] == "AV" && !is_numeric($_POST['freqcardin']))
			$lblfqcd = "class='erro'";		
		$obscdvs = $fachada->printOBS($lecdvs->obs, (isset($_POST['obscdvs']) ? $_POST['obscdvs'] : ''));
    ...
    case "exame_apresp":
		...
		$obsresp = $fachada->printOBS($lersp->obs, (isset($_POST['obsresp']) ? $_POST['obsresp'] : ''));
    ...
    case "exame_apgast":
		...
		$obsgi = $fachada->printOBS($legt->obs, (isset($_POST['obsgi']) ? $_POST['obsgi'] : ''));
    ...
    case "exame_genurin":
		...
		$obsgen = $fachada->printOBS($legu->obs, (isset($_POST['obsgen']) ? $_POST['obsgen'] : ''));
    ...
    case "exame_muscesq":
		...
		$obsmusc = $fachada->printOBS($leme->obs, (isset($_POST['obsmusc']) ? $_POST['obsmusc'] : ''));
    ...
    case "exame_nervoso":
		...
		$obsnerv = $fachada->printOBS($lenv->obs, (isset($_POST['obsnerv']) ? $_POST['obsnerv'] : ''));
    ...
    case "hd":
		...
		if (isset($msgi)) {
			...
			if (isset($hipotese)) 
				$hipdiag = $fachada->printOBS($_POST['hipdiag'], $hipotese->hipotese);
		}
		$listahip = $fachada->listaHD($atend);
		if ($msge && !$_POST['hipdiag'])
			$lblhd = "class='erro'";
		$titulopront = "Problemas";
		break;
    default:
        $div = "div_paciente";
        $dir = "aba_informante";
		...
		if (isset($_POST['nome']) || isset($informante->nome)) {
...
if (is_object($paciente)) {
	...
	if(strlen($paciente->getNome())>30){
		...
		while(($tamanho + strlen(current($array))) < 30){
			$name .= current($array).' ';
			$tamanho = $tamanho + strlen(current($array)) + 1;
			next($array);
		}
	}else{
	...
	$cidadePac = $fachada->getCidade($paciente->getCidade());
	$estadoPac = $fachada->getCidade($paciente->getCidade());
	$natural = $cidadePac['nome']."/".$estadoPac['uf'];
