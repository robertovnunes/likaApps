...
if (isset($_POST['ctrlhp']) and $_POST['ctrlhp'])
	$ctrlhp = $_POST['ctrlhp'];
else 
	$ctrlhp = 1;
	...
	if ($fachada->verPreenchimento($ccaba,'3')){
		try {
			$fachada->inserirNeoNatalHP($_POST['apgar1'], $_POST['apgar5'], $_POST['permnasal'], 
				...
				$_POST['pesoat'], $_POST['spesoat'], $_POST['altat'], $pront);
...
$obsaliment = $fachada->printOBS($lalim->obs, (isset($_POST['obsaliment']) ? $_POST['obsaliment'] : ''));
/* ------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------ Crescimento e Desenvolvimento ------------------------------------------ */
/* ------------------------------------------------------------------------------------------------------------- */
$lcd = $fachada->getValuesTablePr('crescimento',$pront);
$relirm = $fachada->printOBS($lcd->relirm, (isset($_POST['relirm']) ? $_POST['relirm'] : ''));
...
$aprend = $fachada->printOBS($lcd->aprend, (isset($_POST['aprend']) ? $_POST['aprend'] : ''));
$repetano = $fachada->printOBS($lcd->repetano, (isset($_POST['repetano']) ? $_POST['repetano'] : ''));
$relacol = $fachada->printOBS($lcd->relacol, (isset($_POST['relacol']) ? $_POST['relacol'] : ''));
$partfala = $fachada->printOBS($lcd->partfala, (isset($_POST['partfala']) ? $_POST['partfala'] : ''));
$muddent = $fachada->printOBS($lcd->muddent, (isset($_POST['muddent']) ? $_POST['muddent'] : ''));
$obscrescdes = $fachada->printOBS($lcd->obs, (isset($_POST['obscrescdes']) ? $_POST['obscrescdes'] : ''));
...
$talerg = $fachada->printOBS($ldoenc->talerg, (isset($_POST['talerg']) ? $_POST['talerg'] : ''));
...
$trata = $fachada->printOBS($ldoenc->trata, (isset($_POST['trata']) ? $_POST['trata'] : ''));
$obsdoenant = $fachada->printOBS($ldoenc->obs, (isset($_POST['obsdoenant']) ? $_POST['obsdoenant'] : ''));
if (isset($_POST['alerg']) and isset($_POST['talerg'])) {
...
$obshab = $fachada->printOBS($lhab->obs, (isset($_POST['obshab']) ? $_POST['obshab'] : ''));
/* ------------------------------------------------------------------------------------------------------------- */
/* -------------------------------------------- Histórico Familiar --------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------- */
...
if ($error && $_POST['profpai'] == "0")
	$ocuppai = 'class="erro"';	*/
$idademae = $fachada->printCampo($linfam->idademae, (isset($_POST['idademae']) ? $_POST['idademae'] : ''));
$idadepai = $fachada->printCampo($linfam->idadepai, (isset($_POST['idadepai']) ? $_POST['idadepai'] : ''));
$pais = $fachada->printOBS($linfam->pais, (isset($_POST['pais']) ? $_POST['pais'] : ''));
$avos = $fachada->printOBS($linfam->avos, (isset($_POST['avos']) ? $_POST['avos'] : ''));
$irmaos = $fachada->printOBS($linfam->irmaos, (isset($_POST['irmaos']) ? $_POST['irmaos'] : ''));
$outros = $fachada->printOBS($linfam->outros, (isset($_POST['outros']) ? $_POST['outros'] : ''));
/*if ($errhf && !$_POST['avos'])
	$lblavos = 'class="erro"';
...
$comod = $fachada->printCampo($lindom->comod, (isset($_POST['comod']) ? $_POST['comod'] : ''));
$ocup = $fachada->printCampo($lindom->ocup, (isset($_POST['ocup']) ? $_POST['ocup'] : ''));
...
$obsconddom = $fachada->printOBS($lindom->obs, (isset($_POST['obsconddom']) ? $_POST['obsconddom'] : ''));
/* ------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------- Imunizações ----------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------- */
...
$efeitcol = $fachada->printOBS($limun->efeitcol, (isset($_POST['efeitcol']) ? $_POST['efeitcol'] : ''));
...
$obsimun = $fachada->printOBS($limun->obs, (isset($_POST['obsimun']) ? $_POST['obsimun'] : ''));
/* ------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------- Natal ----------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------- */
...
if (isset($errhf)) {
	if (isset($errnt) && !$fachada->validaHora($_POST['durparto']))
		$lbltrpart =  'class="erro"';
}
$durparto = $fachada->printCampo($lnat->durparto, (isset($_POST['durparto']) ? $_POST['durparto'] : ''));
...
if (isset($errhf)) {
	if (isset($errnt) && $_POST['complp'] == "S" && !$_POST['tcomplp']) 
		$lblcomp = 'class="erro"';
}
...
$analg = $fachada->printOBS($lnat->analg, (isset($_POST['analg']) ? $_POST['analg'] : ''));
$tcomplp = $fachada->printOBS($lnat->tcomplp, (isset($_POST['tcomplp']) ? $_POST['tcomplp'] : ''));
$obsnatal = $fachada->printOBS($lnat->obs, (isset($_POST['obsnatal']) ? $_POST['obsnatal'] : '')); 
/* ------------------------------------------------------------------------------------------------------------- */
/* ----------------------------------------------- Neo-Natal --------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------- */
...
$spesoat1 = $fachada->checkOption("spesoat","NA", $lnnat->spesoat);
$spesoat2 = $fachada->checkOption("spesoat","AV", $lnnat->spesoat);


$pesoat = $fachada->printOBS($lnnat->pesoat, (isset($_POST['pesoat']) ? $_POST['pesoat'] : ''));
$altat = $fachada->printOBS($lnnat->altat, (isset($_POST['altat']) ? $_POST['altat'] : ''));
$pesonasc1 = $fachada->checkOption("spesonasc", "NA", $lnnat->pesonasc);
$pesonasc2 = $fachada->checkOption("spesonasc", "AV", $lnnat->pesonasc);
$pesonasc = $fachada->printOBS($lnnat->vpesonasc, (isset($_POST['pesonasc']) ? $_POST['pesonasc'] : ''));
if (isset($errhf)) {
	if (isset($errnnt) && $_POST['spesonasc'] == "AV" && !is_numeric($_POST['pesonasc'])) 
		$lblpeson = "class='erro'";
}
...
$obsnnatal = $fachada->printOBS($lnnat->obs, (isset($_POST['obsnnatal']) ? $_POST['obsnnatal'] : ''));
/* ------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------ Outras Informações ----------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------- */
...
$tagtox = $fachada->printOBS($linfo->tagtox, (isset($_POST['tagtox']) ? $_POST['tagtox'] : ''));
...
$talergmed = $fachada->printOBS($linfo->talergmed, (isset($_POST['talergmed']) ? $_POST['talergmed'] : ''));
$obsoutinfo = $fachada->printOBS($linfo->obs, (isset($_POST['obsoutinfo']) ? $_POST['obsoutinfo'] : ''));
/* ------------------------------------------------------------------------------------------------------------- */
/* ----------------------------------------------- Pré-Natal --------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------- */
...
$lacpmed = $fachada->printOBS($lpnat->lacpmed, (isset($_POST['lacpmed']) ? $_POST['lacpmed'] : ''));
...
//	$lbMedic = 'class="erro"';
$medic = $fachada->printOBS($lpnat->medic, (isset($_POST['medic']) ? $_POST['medic'] : ''));
if (isset($errpnt)) {
	if ($errpnt && !$linha->ass && $_POST['anorm'] == "") 
		$lbAnorm = 'class="erro"';
}
$anorm = $fachada->printOBS($lpnat->anorm, (isset($_POST['anorm']) ? $_POST['anorm'] : ''));
...
$cirurgdc = $fachada->printOBS($lpnat->cirurgdc, (isset($_POST['cirurgdc']) ? $_POST['cirurgdc'] : ''));
...
$pesograv = $fachada->printCampo($lpnat->pesograv, (isset($_POST['pesograv']) ? $_POST['pesograv'] : ''));
$obsprenat = $fachada->printOBS($lpnat->obs, (isset($_POST['obsprenat']) ? $_POST['obsprenat'] : ''));
/* ------------------------------------------------------------------------------------------------------------- */
