...
<?php
    ...
    if ($error)
		...
		if ((isset ($_POST['buscar'])) and ($_POST['buscar'] == "Buscar" && sizeOf($listacons) == 0))
			echo '<div class="msgerro">O paciente consultado n&atilde;o est&aacute; dispon&iacute;vel ou n&atilde;o existe.</div>';
		else if ((isset ($_POST['buscar'])) and ($_POST['buscar'] == "Buscar" && sizeOf($listacons) > 0))
...
<?php } ?>	
    <form action="aluno.php" method="post" name="conspac" >
        <fieldset class="fieldconsulta">
            ...
            <label id="cEsq"><input name="valor" value="<?php echo ((isset ($_POST['valor'])) ? $_POST['valor'] : ''); ?>" size="50" maxlength="30" /></label>
            <label id="cCen">
                <select name="tipo">
  				    <option value="n" <?php echo ((isset ($a)) ? $a : ''); ?>>Nome</option>
  				    <option value="p" <?php echo ((isset ($b)) ? $b : ''); ?>>N&ordm; de Prontuario</option>
  				    <option value="m" <?php echo ((isset ($c)) ? $c : ''); ?>>Nome da M&atilde;e</option>
  	  		    </select>
            </label>
            <label id="cDir"><input name="buscar" type="submit" value="Buscar" /></label>
        </fieldset>
    </form>
<?php if ((isset ($listacons)) and (sizeOf($listacons) > 0)) { ?>
    <br />
	<table id="tableConsultaPaciente">
    <thead class="bgsTCP">
		<tr class="styleTabelaConsultaPaciente stylebold">
