...
<fieldset class="full">
	...
	<?php foreach ($vacpac as $lvac){?>
		<tr class="style51">
			...
			<td><input type="image" value="Remover Vacina" name="vac" src="images/lixo.gif" onclick="removerVacina('<?php echo $lvac['vacina'];?>', '<?php echo $lvac['dose'];?>');" /></td>
		</tr>
	<?php } ?>
	</table>
...
<fieldset class="full">
<legend id="lblnvac" <?php if ($errvac){ echo 'class="erro"'; }?>>Inserir Nova Vacina</legend>
	<table width="560" align="center">
		<tr class="style51">
			<td>Vacina</td>
			<td>Dose</td>
		...
		<tr class="style51">
			<td>
				...
				<?php foreach ($vacinas as $nvac){
					if ($nvac['idVacina'] == $_POST['vacinanome']) $sel = "SELECTED"; else $sel = ""; ?>
			...
			<td>
				...
				<?php foreach ($dose as $doses){
					if ($doses['idDose'] == $_POST['vacinadose']) $sel = "SELECTED"; else $sel = ""; ?>
					<option value="<?php echo $doses['idDose']; ?>" <?php echo $sel; ?>><?php echo $doses['dose']; ?></option>
				<?php } ?>
				</select>
			</td>
			<td>
				<input name="datavacina" size="12" maxlength="10" onkeypress="mascara(this,2)" onfocus="mudaLb(lblnvac)" value="<?php echo $_POST['datavacina']; ?>" onkeyup="savCampo()" />
