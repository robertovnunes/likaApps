...
class syntax_plugin_countdown extends DokuWiki_Syntax_Plugin {
    ...
    function getInfo() {
        global $lang;
        $this_info= parent::getInfo();
        $that_lang= $this->getLang('desc');
        if ($this_info['desc'] !== $that_lang) $this_info['desc'] = $that_lang;
        return $this_info;
    }
    ...
    function render($mode, &$renderer, $data) {
        ...
        $thatDate = strtotime ($data[0]);
        if ($thatDate <= 0) {
            $renderer->doc .= $this->getLang('wrongformat'). ': '. $data[0]. '.';
            return true;
        }
        // get the time difference and present it via the renderer
        $difdata= $this->compute($thatDate);
        if ('xhtml' === $mode) {
            $renderer->doc .= '<!-- countdown begin '. $data[0]. '-->';
            $renderer->doc .= '<span class="countdown">';
            if (($difdata['distance'] == 0) && $this->getConf('use_today') ) {
                // today is...
                $renderer->doc .= $this->getLang('today');
            } else {
                // ... day[s] ...
                $renderer->doc .= sprintf("%u", $difdata['days']) . ' ';
            ...
            $pdesc= p_render('xhtml', p_get_instructions($description), $info);
...
*/
    ...
    **/
    private function compute ($targetdate) {
        ...
        $diffseconds = $targetdate - time(void);
        $difference = $diffseconds / 86400; 

        return array(
            'distance' => $difference,
            'days'     => abs(($difference)),
            'bdays'    => $this->bdaysbetween( time(void), $targetdate, array() ),
