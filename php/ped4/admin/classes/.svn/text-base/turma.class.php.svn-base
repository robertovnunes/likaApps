...
class Turma
{
	...
	public function insert()
	{
		...
		$query = sprintf("insert into turma (turma,per) values('%s','%s')", mysql_real_escape_string($this -> nome), mysql_real_escape_string($this -> periodo));
	...
	public function edit()
	{
		...
		$query = sprintf("update turma set turma = '%s', per = '%s' where idTurm = %d",mysql_real_escape_string($this -> nome), mysql_real_escape_string($this -> periodo), $this -> id);
		$result = mysql_query($query);
	...
	public function delete()
	{
		...
		$query = sprintf("delete from turma where idTurm = %d", $this -> id);
		$result = mysql_query($query);
	...
	public static function find($array)
	{
		...
		{
			array_push($temp,"per LIKE '%".mysql_real_escape_string($array['periodo'])."%'");
		}
